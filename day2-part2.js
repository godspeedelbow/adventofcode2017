const rows = getSpreadsheetString()
  // map string to list of rows
  .split('\n')

  // map each row to list of numbers
  .map(row => row
     .split('\t')
     .map(n => parseInt(n, 10))
  );
...
const checksum = rows
  ...
  .map(row => row
      // map row to divisons
      .map(getEvenlyDivisiblePeer)
       
      // reduce to the valid division 
      .find(n => n)
  )
  // sum divisions up
  .reduce((agg, division) => agg + division, 0);

console.log('checksum', checksum);

function getEvenlyDivisiblePeer(number, index, numbers) {
  ...
  const peer = without
    // calculate modulo for each number in row
    .map(n => number % n)
  
    // find modulo that is 0
    .findIndex(m => m === 0);

  // return division if there's one
  return peer < 0
    ? 0
    : Math.max(number, without[peer]) / Math.min(number, without[peer]);
}

function getSpreadsheetString() {
  return `121	59	141	21	120	67	58	49	22	46	56	112	53	111	104	130
